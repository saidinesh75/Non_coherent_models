...
def psk_constel(K):
    ...
    if K == 2:
        ...
        c = torch.tensor([1+0j, 0+1j, -1+0j, 0-1j]).type(torch.cdouble)
    else:
        theta = 2*torch.pi*torch.arange(K)/K
        c     = (torch.cos(theta) + 1J*torch.sin(theta)).type(torch.cdouble)
...
def psk_mod(bin_arr, K):
    ...
    assert list(bin_arr.size())[0] % logK == 0
    L    =  list(bin_arr.size())[0] // logK     # Number of symbols
    if L == 1:
        k = list(bin_arr.size())[0]
        idx     = gray2bin(bin_arr_2_int(bin_arr)) # gray code index
        symbols = c[idx]
    else:
        symbols = torch.zeros(L, dtype=torch.cdouble)
        for l in range(L):
...
def gen_msg_mod_torch(code_params,cols):
